...
export type TabType = 'active' | 'coming' | 'completed' | 'break' | 'deleted' | 'shop';
	...
	return (
		<nav className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-200 z-50 px-2 sm:px-6 py-2 flex justify-between items-center text-xs font-semibold pb-safe">
			<div className="max-w-7xl mx-auto w-full flex justify-center sm:justify-between">
				{displayTabs.map(tab => (
					<button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 min-w-15 flex flex-col items-center gap-1 transition-all ${activeTab === tab ? 'text-orange-500 scale-105' : 'text-gray-400 hover:text-dark'}`}>
						<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>{TAB_CONFIG[tab].icon}</svg>
				...
				{menuTabs.length > 0 && (
					<div className="flex-1 min-w-15 flex justify-center relative" ref={moreMenuRef}>
						<button onClick={() => setIsMoreMenuOpen(!isMoreMenuOpen)} className={`flex flex-col items-center gap-1 transition-all ${menuTabs.includes(activeTab) ? 'text-orange-500 scale-105' : 'text-gray-400 hover:text-dark'}`}>
